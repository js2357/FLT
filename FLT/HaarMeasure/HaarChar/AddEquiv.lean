...
/-   -- Define the measure `ν`
  ...
  --------------------------------------------------------------------------------------------
  -- The necessary API for ν and μ which is mostly proven in the commented-out code above.
  ...
  suffices mulEquivHaarChar (φ.prodCongr ψ) * haar (X ×ˢ Y) =
    ...
    have ne_top : haar (X ×ˢ Y) ≠ ⊤ := by
      apply LT.lt.ne ∘ lt_of_le_of_lt (measure_mono <| Set.prod_mono hX hY)
      exact (isHaarMeasure_haarMeasure _).lt_top_of_isCompact <| hK'comp.prod hKcomp
    have μ0 : haar (X ×ˢ Y) ≠ 0 :=
      (isHaarMeasure_haarMeasure _).open_pos _ hXYopen ⟨⟨1, 1⟩, ⟨one_mem_X, one_mem_Y⟩⟩
    exact_mod_cast (ENNReal.mul_left_inj μ0 ne_top).mp this

  calc mulEquivHaarChar (φ.prodCongr ψ) * haar (X ×ˢ Y)
    _ = mulEquivHaarChar (φ.prodCongr ψ) *
          (map (φ.prodCongr ψ) haar) ((φ.prodCongr ψ) '' (X ×ˢ Y)) := by
    ...
    _ = ν (φ '' X) := by
      rw [ν_apply]; exact φ.measurableEmbedding.measurableSet_image' hXopen.measurableSet
    ...
    _ = (mulEquivHaarChar φ) * ν X := by
      rw [map_apply, show φ ⁻¹' (φ '' X) = X from φ.preimage_image X]
      · exact φ.measurable
      · exact (φ.isOpen_image.mpr hXopen).measurableSet
